...
if (isset($_POST['jobPublish'])) {
    if (
        ...
        if ($req_job_verif->rowCount() > 0) {
            ...
            try {
                $req_insert_job = $bdd->prepare("INSERT INTO recrutement (job_title,job_description,job_location,job_time,job_contract,job_device,job_salary,job_offer_author,job_identification) VALUES (?,?,?,?,?,?,?,?,?)");
                $req_insert_job->execute([$job_title, $job_description, $job_location, $job_time, $job_contract, $job_device, $job_mount, $job_author, $job_identity_number]);
            } catch (PDOException $e) {
